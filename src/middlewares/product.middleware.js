...
export const checkExpirationDate = async (req, res, next) => {
...
export const checkIfImageChanged = async (req, res, next) => {
	...
	if (oldImages.length + newImages.length < 4) {
		return res.status(400).json({ message: 'Images must be at least 4' });
	}
	req.oldImages = oldImages;
	req.newImages = newImages;
	return next();
};

export const uploadChangedImages = async (req, res, next) => {
	const oldImages = req.oldImages;
	const newImages = req.newImages;
	const uploadedImages = [...oldImages];
	for (let i = 0; i < newImages.length; i++) {
		const { url } = await uploadPhoto(req, res, newImages[i]);
