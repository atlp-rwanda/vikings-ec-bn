import {knownNotificationType, NotificationService} from '../services/notification.service';
...
export const notifyProductLovers = async (productId, type, message) => {
...
const getProductSale =  async(sales) => {
    return Promise.all(sales.map(sale => {
        return new Promise((resolve, reject) => {
            ProductService.getProductById(sale.productId).then(product =>{
                resolve({product,sale});
            }).catch(reject);
        });
    }));
};
export const notifySellersOnBuyProduct = async (sales, buyerId) => {
    eventEmit(knownEvents.onNotification, {
        type:knownNotificationType.newOrder,
        message:' Your order received successful. we will keep you in touch ',
        receiverId: buyerId,
    });
    const salesProduct = await getProductSale(sales);
...
export const notifySellersAboutOrder = async (sales, status) => {
    const salesProduct = await getProductSale(sales);
    salesProduct.map(each => {
        return eventEmit(knownEvents.onNotification, {
...
export const notifyBuyerAboutOrder = async (orderId,status) => {
