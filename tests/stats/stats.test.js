...
describe('statistics', () => {
	...
	test('Good request and getting the stats', async () => {
		const response = await request(app)
			.get(
				`/api/v1/stats/?start=${start}&end=${end}&include=sales,expired,wishes,product-created`
			)
	...
	test('out of range or non-existing date status: 400', async () => {
		const response = await request(app)
			.get(
				`/api/v1/stats/?start=${start}&end=${end}&include=solas,expired,wishes,product-created`
			)
			.set('Authorization', `Bearer ${sellerToken}`)
			.send();
		expect(response.statusCode).toEqual(400);
