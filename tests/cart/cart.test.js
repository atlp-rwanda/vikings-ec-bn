...
describe('/cart', ()=>{
  test('No authorization: 400 and unauthorized message', async ()=>{
    const response = await request(app).post('/api/v1/carts/').send(randomProductId);
  ...
  test('No product: 400 and required message', async ()=>{
    const response = await request(app).post('/api/v1/carts/').set('Authorization', `Bearer ${buyerToken}`).send();
  ...
  test('New Product: 200', async()=>{
    const response = await request(app).post('/api/v1/carts/').set('Authorization', `Bearer ${buyerToken}`).send(jordan);
  ...
  test('Unavailable Product: 404', async() =>{
    const response = await request(app).post('/api/v1/carts/').set('Authorization', `Bearer ${buyerToken}`).send(randomProductId);
